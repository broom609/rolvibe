  ...
  return (
    <div>
      <div className="flex items-center justify-between mb-6">
        <h1 className="text-2xl font-bold text-[var(--text-primary)]">My Apps</h1>
        <Link href="/dashboard/submit" className="btn-primary text-sm">Submit New App</Link>
      </div>

      {appList.length === 0 ? (
        <div className="bg-[var(--card)] border border-[var(--border)] rounded-xl p-12 text-center">
          <p className="text-[var(--text-secondary)] mb-4">You haven&apos;t submitted any apps yet.</p>
              ...
              {appList.map(app => (
                <tr key={app.id} className="hover:bg-[var(--card-hover)] transition-colors">
                  <td className="px-4 py-3">
                    <div>
                      <p className="font-medium text-[var(--text-primary)] max-w-[180px] truncate">{app.name}</p>
                      <p className="text-xs text-[var(--text-muted)] max-w-[180px] truncate">{app.tagline}</p>
